...
from easyfsl.methods import NEWMatchingNetworks,FewShotClassifier   #
# 获取模型名称
...
# few_shot_classifier=NEWMatchingNetworks(modify_last_layer(model_name, 20), feature_dimension=int(feature_dimension))
...
# selected_layers = ['layer3.1.conv1', 'layer4.0.conv2']
selected_layers = [
...
def train_and_validate(few_shot_classifier, train_loader, val_loader,learning_rates,lambda_center,n_epochs, visualize=False):
    ...
    # # 保存 ROC 曲线图像
    roc_filename = f'roc_curve_mini_withreg_newmatch_{lambda_center}_{n_way}_{n_shot}.png'
    plot_roc_curve_single(all_true_labels, all_predicted_probs, save_path=roc_filename)
    #保存roc结果
    save_data_path = f'roc_curve_mini_withreg_newmatch_{lambda_center}_{n_way}_{n_shot}.npz'
    np.savez(save_data_path, true_labels=all_true_labels, predicted_probs=all_predicted_probs)
    
    # 可视化 t-SNE
    if visualize:
        tsne_filename = f'tsne_visualization_diy_mini_{lambda_center}_{n_way}_{n_shot}.png'
...
save_results_to_json(results, f'shallow_mini_newmatch_prune_step_{args.lambda_center}_{args.lr}_{args.shot}shot.json')
