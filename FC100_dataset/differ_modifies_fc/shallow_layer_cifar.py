...
import easyfsl
from easyfsl.datasets import CIFAR
...
train_set = CIFAR(split="train", training=True, transform=transform)
val_set = CIFAR(split="val", training=False, transform=transform)
...
from easyfsl.methods import NEWMatchingNetworks,FewShotClassifier   #BDCSPN, FewShotClassifier
# 获取模型名称
...
def train_and_validate(few_shot_classifier, train_loader, val_loader,learning_rates,lambda_center,n_epochs, visualize=False):
    results = {
        "train_loss": [],
        "train_accuracy": [],
        "val_loss": [],
    ...
    for epoch in range(n_epochs):
        ...
        # 更新最佳模型：如果验证损失比当前的最佳损失低，则更新  用acc loss还是val loss??
    ...
    test_set = CIFAR(split="test", training=False)
    ...
    # print(f"Average org test accuracy: {(100 * accuracy):.2f} %")
    # 测试集评估并计算 ROC 曲线
    # test_accuracy,all_true_labels, all_predicted_probs = evaluate_roc(few_shot_classifier, test_loader, device=DEVICE)
    ...
    # # 保存 ROC 曲线图像
    roc_filename = f'roc_curve_cifar_withreg_newmatch{args.lambda_center}_{n_way}_{n_shot}.png'
    plot_roc_curve_single(all_true_labels, all_predicted_probs, save_path=roc_filename)
    #保存roc结果
    save_data_path = f'roc_curve_cifar_withreg_newmatch{args.lambda_center}_{n_way}_{n_shot}.npz'
    np.savez(save_data_path, true_labels=all_true_labels, predicted_probs=all_predicted_probs)

    
    # 可视化 t-SNE
    if visualize:
        tsne_filename = f'tsne_visualization_diy_cifar_{lambda_center}_{n_way}_{n_shot}.png'
...
few_shot_classifier = NEWMatchingNetworks(
        change_network, 
        feature_dimension=int(args.feature_dim), 
        inter_class_distance_weight=args.lambda_center, 
    )
total_params = sum(p.numel() for p in few_shot_classifier.parameters() if p.requires_grad)
print(f"Total trainable parameters: {total_params}")

# 训练并评估模型
results,test_acc = train_and_validate(few_shot_classifier, train_loader, val_loader, args.lr,args.lambda_center,n_epochs=50,visualize=False)
print(f"Parameters of this epoch: {params}")

save_results_to_json(results, f'shallow_cifar_newmatch_prune_step_{args.lambda_center}_{args.lr}_{args.shot}shot.json')
