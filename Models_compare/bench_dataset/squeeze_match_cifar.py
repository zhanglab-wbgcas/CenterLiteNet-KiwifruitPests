...
def train_and_validate(few_shot_classifier, train_loader, val_loader,learning_rates,n_epochs, visualize=False):
    ...
    # # 保存 ROC 曲线图像
    roc_filename = f'roc_curve_cifar_match_{args.pretrain_model}_{n_way}_{n_shot}.png'
    plot_roc_curve_single(all_true_labels, all_predicted_probs, save_path=roc_filename)
    #保存roc结果
    save_data_path = f'roc_curve_cifar_match_{args.pretrain_model}_{n_way}_{n_shot}.npz'
    np.savez(save_data_path, true_labels=all_true_labels, predicted_probs=all_predicted_probs)

    
    # 可视化 t-SNE
    if visualize:
        tsne_filename = f'tsne_visualization_cifar_match_{args.pretrain_model}_{n_way}_{n_shot}.png'
...
save_results_to_json(results, f'match_step_cifar_{args.pretrain_model}_{args.lr}_{args.shot}shot.json')
