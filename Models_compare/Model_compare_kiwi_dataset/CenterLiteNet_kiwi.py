...
from easyfsl.utils import evaluate,evaluate_on_one_task,evaluate_loss,evaluate_ontest,visual_ontest_with_images
# visual_ontest,
...
def train_and_validate(few_shot_classifier, train_loader, val_loader,learning_rates,lambda_center,n_epochs, visualize=False):
    ...
    # 可视化 t-SNE
    if visualize:
        tsne_filename = f'newtsne_layer03_kiwi_{lambda_center}_{n_way}_{n_shot}.png'
        # visual_ontest(few_shot_classifier, test_loader, device=DEVICE, tqdm_prefix="Test", visualize=True, save_path=tsne_filename)
        visual_ontest_with_images(few_shot_classifier, test_loader, device=DEVICE, tqdm_prefix="Test", visualize=True, save_path=tsne_filename)
...
# 训练并评估模型
results,test_acc = train_and_validate(few_shot_classifier, train_loader, val_loader, args.lr,args.lambda_center,n_epochs=50,visualize=True)
